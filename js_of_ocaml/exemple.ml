...
let clicked i deg=
  ...
  deg##.innerHTML := Js.string(string_of_float (float_of_string(c)*.1.8+.32.0)^"°F") (*On transforme la valeur string Caml en valeur float puis dans le champs innerHTML de la balise <p> on place la valeur en float modifiée et remise en Js String *)

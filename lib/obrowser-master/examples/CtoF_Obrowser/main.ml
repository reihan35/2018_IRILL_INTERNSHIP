...
let _ =
  let body = Node.document >>> get "body" in (*On récupère le noeud correspondant à l'élément body*)
   ...
   Js.Fragment.flush body (button "Convertis !" "clickme" (fun() ->let x = Sys.time() in
                                                                   (Node.remove count3 (Node.child count3 0));
                                                                   (Node.append count3 (Node.text ((string_of_float((float_of_string(Js.Node.get_attribute b "value"))*.1.8+.32.0))^"°F")));
                                                                   alert(string_of_float(Sys.time() -. x))));

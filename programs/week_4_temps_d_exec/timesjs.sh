...
(time -p ocamlfind ocamlc js_affiche.ml -o js_affiche.byte -package js_of_ocaml -package js_of_ocaml-ppx -linkpkg | tail -n 3 | head -n 1) > outfile 2>>time.txt
...
(time -p npm run build ) > outfile 2>>time.txt
...
(time -p make) > outfile 2>>time.txt
